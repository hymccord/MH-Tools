...
const districtMice = {
  Farming: ["Mush Monster", "Mushroom Harvester", "Nightshade Nanny"],
  Fealty: [
    "Dark Templar",
    "Drudge",
    "Masked Pikeman",
  ...
  Treasury: ["Hired Eidolon", "Mimic", "Treasure Brawler"]
};
  ...
  return [
    {
      ...
      config: [
        {
          opts: {
            include: [...allHallsMice, ...districtMice[hall]]
          }
        }
      ]
    },
...
function genHallways() {
  ...
  const districtsWithEpic = ["Plain", "Superior", "Epic"]
    .map(quality =>
      ["Fealty", "Tech", "Scholar"].map(hallway =>
        genSeriesObject(quality, hallway)
      )
    )
    .flat(2);

  // These don't have an Epic hallway and will cause a query error b/c stage doesn't exist.
  const nonEpic = ["Plain", "Superior"]
    .map(quality =>
      ["Farming", "Treasury"].map(hallway => genSeriesObject(quality, hallway))
    )
    .flat(2);

  return [...districtsWithEpic, ...nonEpic];
}
